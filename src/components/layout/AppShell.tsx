...
export function AppShell({ children, guideSections = [] }: { children: React.ReactNode; guideSections?: GuideSection[] }) {
  const { isSidebarOpen, setSidebarOpen, isUtilityPanelOpen, setUtilityPanelOpen, toggleUtilityPanel } = useAppShell();
    ...
    <div className="min-h-screen bg-[var(--bg)] flex flex-col md:flex-row font-sans text-[var(--text)]">
      ...
      <main className="flex-1 flex flex-col min-w-0 transition-all duration-300 relative max-w-full md:max-w-none">
        ...
        <header className="hidden md:flex h-14 border-b border-[var(--line)] bg-[var(--surface)] items-center px-6 justify-between shrink-0 sticky top-0 z-10 w-full">
          <div className="text-sm font-medium text-[var(--text-muted)] flex items-center space-x-4">
             ...
             <div className="px-3 py-1.5 bg-[var(--surface-soft)] rounded-md cursor-pointer hover:bg-[var(--line)] transition-colors flex items-center space-x-2">
               <span>搜尋此指南</span>
               <span className="text-xs bg-[var(--bg)] px-1 rounded border border-[var(--line)]">Cmd+K</span>
             </div>
          </div>
          <div className="flex items-center space-x-2">
             <button onClick={toggleUtilityPanel} className="p-2 text-[var(--accent-primary)] hover:bg-[var(--surface-soft)] rounded-full transition-colors xl:hidden" title="Toggle Utility Panel">
                {isUtilityPanelOpen ? <PanelRightClose className="w-5 h-5" /> : <PanelRightOpen className="w-5 h-5" />}
             </button>
          </div>
        </header>
        ...
        <div className="flex-1 w-full max-w-[860px] mx-auto p-4 md:p-8 lg:p-12 pb-24 relative overflow-y-auto">
          {children}
        </div>
      ...
      <>
        ...
        <aside 
           ...
           ${isUtilityPanelOpen ? 'translate-x-0' : 'translate-x-full xl:translate-x-0'}`}
        >
          <div className="p-4 flex items-center justify-between border-b border-[var(--line)]">
             ...
             {activeSection && (
               <div className="pt-4 border-t border-[var(--line)] space-y-2">
                 ...
                 <div className="h-2 w-full bg-[var(--line)] rounded-full overflow-hidden">
                   <div className="h-full bg-[var(--accent-primary)] w-0 transition-all duration-300" style={{ width: '0%' }}></div>
                 </div>
                 <p className="text-xs text-[var(--text-muted)]">0% 已完成</p>
               </div>
             )}
          </div>
        </aside>
